...
const envSchema = z.object({
  ...
  PORT: z.preprocess(
    (val) => Number(val),
    z.number().positive().default(8080)
  ),
  ...
  TEMP_UPLOAD_DIR: z.string().optional().default(path.join(process.cwd(), 'server', 'uploads')),
  MAX_FILE_SIZE: z.preprocess(
    (val) => Number(val),
    z.number().positive().default(10 * 1024 * 1024) // 10MB
  ),
