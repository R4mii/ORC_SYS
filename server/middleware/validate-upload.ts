...
const fileFilter = (req: Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
  ...
  if (!CONFIG.upload.allowedMimeTypes.includes(file.mimetype)) {
    return cb(new AppError(`Invalid file type. Allowed types: ${CONFIG.upload.allowedMimeTypes.join(", ")}`, 400))
  }
  ...
  if (!validExt) {
    return cb(new AppError(`File extension doesn't match the file type`, 400))
  }
