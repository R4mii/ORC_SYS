...
import { validateFileContent } from "@/lib/utils/file-validation"
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Validate the file using more robust content validation
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          return NextResponse.json(
            {
              error: errorJson.error || "OCR service error",
              details: errorJson.details || errorText.substring(0, 200),
            },
            { status: response.status },
          )
        } catch (parseError) {
          // If JSON parsing fails, return a generic error with the raw error text
          return NextResponse.json(
            { error: "OCR service error", details: errorText.substring(0, 200) },
            { status: response.status },
          )
        }
      }
